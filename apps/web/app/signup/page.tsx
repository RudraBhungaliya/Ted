...
export default function SignupPage() {
  ...
  return (
    <main className="min-h-screen bg-[#090D1A] text-zinc-100 flex items-center justify-center relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md p-8 rounded-2xl bg-neutral-900/40 backdrop-blur-xl border border-white/[0.06] shadow-2xl relative z-10">
        <div className="flex items-center gap-3 justify-center mb-8">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-white">Ted</h1>
        </div>

        <h2 className="text-xl font-semibold text-center mb-6">Create Account</h2>

        {error && (
          <div className="mb-4 p-3.5 rounded-xl border border-red-500/20 bg-red-500/5 text-red-400 text-xs text-center">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm transition-all hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 cursor-pointer shadow-lg shadow-indigo-500/20"
          >
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs uppercase"><span className="bg-[#090D1A]/0 px-2 text-zinc-500 font-semibold tracking-wider bg-[#0b0f20]">Or continue with</span></div>
        </div>

        <button
          onClick={handleGoogleLogin}
          type="button"
          className="w-full py-3.5 rounded-xl bg-white/5 border border-white/10 text-zinc-300 font-medium text-sm flex items-center justify-center gap-2.5 hover:bg-white/10 transition-colors cursor-pointer"
        >
