...
export default function DashboardPage() {
  ...
  const maxTrendCount =
    data?.global.weeklyTrend.reduce((max, t) => Math.max(max, t.count), 1) || 1;
      ...
      <main className="min-h-screen bg-[#090D1A] text-zinc-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-400 text-sm font-medium">
            Analyzing intelligence transcripts...
          </p>
        </div>
      </main>
    );
      ...
      <main className="min-h-screen bg-[#090D1A] text-zinc-100 flex items-center justify-center flex-col relative font-sans">
        ...
        <div className="text-center p-8 rounded-2xl bg-neutral-900/40 border border-white/5 backdrop-blur-xl max-w-sm mt-16">
          <p className="text-zinc-400 text-sm mb-4">
            No analytics data available. Complete an interview to view
            dashboard.
          </p>
          <a
            href="/"
            className="inline-block px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 font-bold transition-all text-sm"
          >
            Start Ted Session
          </a>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          <div className="p-6 rounded-2xl bg-neutral-900/40 border border-white/[0.06] backdrop-blur-xl relative overflow-hidden group hover:border-indigo-500/30 transition-all hover:scale-[1.01]">
            ...
            <div className="text-zinc-400 text-xs mt-2">
              Speech pace and certainty score
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-neutral-900/40 border border-white/[0.06] backdrop-blur-xl relative overflow-hidden group hover:border-indigo-500/30 transition-all hover:scale-[1.01]">
            ...
            <div className="text-zinc-400 text-xs mt-2">
              Um, uh, basically, actually counts
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-neutral-900/40 border border-white/[0.06] backdrop-blur-xl relative overflow-hidden group hover:border-indigo-500/30 transition-all hover:scale-[1.01]">
            ...
            <div className="text-zinc-400 text-xs mt-2">
              Total interview records persisted
            </div>
          </div>
            ...
            <div className="flex items-end justify-between h-16 gap-2">
              {data.global.weeklyTrend.map((t, i) => {
                const heightPercent = Math.max(
                  10,
                  Math.round((t.count / maxTrendCount) * 100),
                );
                return (
                  <div
                    key={i}
                    className="flex-1 flex flex-col items-center group/bar cursor-help"
                  >
                    <div
                      className="w-full bg-indigo-500/20 hover:bg-indigo-500/50 rounded-md transition-all relative"
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                  <div className="p-4 rounded-xl border border-white/5 bg-black/25">
                    ...
                    <div className="text-2xl font-extrabold text-white">
                      {selectedSession.metrics.confidenceScore}%
                    </div>
                  </div>
                  <div className="p-4 rounded-xl border border-white/5 bg-black/25">
                    <div className="text-zinc-500 text-[10px] uppercase font-bold tracking-wider mb-1">
                    ...
                    <div className="text-2xl font-extrabold text-white">
                      {selectedSession.metrics.fillerCount}
                    </div>
                  </div>
                  <div className="p-4 rounded-xl border border-white/5 bg-black/25">
                    <div className="text-zinc-500 text-[10px] uppercase font-bold tracking-wider mb-1">
                    ...
                    <div className="text-2xl font-extrabold text-white">
                      {selectedSession.metrics.starUsage}%
                    </div>
                  </div>
                  <div className="p-4 rounded-xl border border-white/5 bg-black/25">
                    <div className="text-zinc-500 text-[10px] uppercase font-bold tracking-wider mb-1">
                    ...
                    <div className="text-2xl font-extrabold text-white">
                      {selectedSession.metrics.wordCount}
                    </div>
                  </div>
                </div>
                ...
                {selectedSession.summary ? (
                  <div className="space-y-6">
                    <div className="flex items-center gap-3 p-4 rounded-xl border border-indigo-500/20 bg-indigo-500/5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-zinc-400 mt-0.5">
                          Overall answer clarity, technical correctness, and
                          pacing index.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <ul className="space-y-2">
                          {selectedSession.summary.strengths.map(
                            (str, index) => (
                              <li
                                key={index}
                      ...
                      <div>
                        ...
                        <ul className="space-y-2">
                          {selectedSession.summary.weaknesses.map(
                            (weak, index) => (
                              <li
                                key={index}
                                className="text-xs text-zinc-300 flex items-start gap-2"
                    ...
                    <div className="pt-4 border-t border-white/5">
                      <h4 className="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-3">
                        Recommended Actions & STAR Analysis
                      </h4>
                      <ul className="space-y-2.5">
                        {selectedSession.summary.recommendations.map(
                          (rec, index) => (
                            <li
                              key={index}
                              className="text-xs text-zinc-300 leading-relaxed flex items-start gap-2.5 bg-white/[0.02] border border-white/5 rounded-xl p-3"
                            >
                  ...
                  <div className="text-center py-12 rounded-xl border border-dashed border-white/10 bg-black/10">
                    ...
                    <h4 className="font-semibold text-zinc-300 text-sm">
                      No Summary Evaluation Available
                    </h4>
